...
In this report we'll discuss: What a database branch is, the different ways they work and how much they cost for the following providers: [Neon](https://neon.tech/), [Supabase](https://supabase.com/) and [PlanetScale](https://planetscale.com/).
...
Like before, there’s no one-size fits all solution to database branching workflows.
...
_It's also worth mentioning that certain database changes can be made without requiring any changes to the codebase, such as adding indexes, creating or modifying views, increasing column lengths, or setting default values._

## Branch creation speeds

Creating a code branch is instant since it’s all normally local. But understandably, setting up a new branch of the database takes longer. How long depends on the technology your database provider uses. For example, if you’re using RDS, launching a new instance and provisioning resources can be a slow process. And as mentioned earlier, creating a schema only branch, setting up a production replica, adding seed data, or anonymizing production data can all add to the time it takes to spin up a database branch.
...
- With [Neon](https://neon.tech/) you get instant branches with schema + data and pretty soon Neon will have schema only branches (i’m going to assume they’ll be instant too, but let’s see)
